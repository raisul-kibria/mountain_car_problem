...
def create_output_dir(dir, alpha, epoch, epsilon_strategy=None, exploratory_strategy=None):
    ...
    """
    OUTPUT_DIR = f"{dir}_alpha_{alpha}_epoch_{epoch}"
    if epsilon_strategy:
        OUTPUT_DIR += f'_{epsilon_strategy}'
    if exploratory_strategy:
        OUTPUT_DIR += f'_{exploratory_strategy}'
    try:
